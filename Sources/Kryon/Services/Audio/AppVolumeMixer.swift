...
final class AppVolumeMixer: ObservableObject {
    ...
    private let buildQueue = DispatchQueue(label: "com.kryon.utils.mixer", qos: .userInitiated)
    ...
    private let halQueue = DispatchQueue(label: "com.kryon.utils.mixer.hal", qos: .userInitiated)
    ...
    private static func outputDevices(defaultUID: String?) -> [MixerOutputDevice] {
        ...
        for deviceID in deviceIDs {
            ...
            let name = read(deviceID, kAudioObjectPropertyName, &nameRef)
                ? nameRef as String
                : uid
            guard name != "Kryon Mixer" else { continue }
...
private final class TapGainEngine: GainEngine {
    ...
    init?(objects: [AudioObjectID], gain: Float, outputDeviceUID: String) {
        ...
        let aggregate: [String: Any] = [
            kAudioAggregateDeviceNameKey: "Kryon Mixer",
    ...
    private static let rateQueue = DispatchQueue(label: "com.kryon.utils.mixer.rate",
                                                 qos: .userInitiated)
    ...
    private static let teardownQueue = DispatchQueue(label: "com.kryon.utils.mixer.teardown",
                                                     qos: .utility)
