...
final class RecorderCaptureEngine: NSObject {
    ...
    private let queue = DispatchQueue(label: "com.kryon.recorder.capture",
                                      qos: .userInitiated)
    ...
    /// Builds the filter and starts the stream. Existing ordinary Kryon
...
final class RecorderMicrophoneCapture: NSObject,
    ...
    private let queue = DispatchQueue(label: "com.kryon.recorder.microphone",
                                      qos: .userInitiated)
