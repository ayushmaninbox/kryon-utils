...
enum UpdateInstallerSupport {
    ...
    static func installerScript() -> String {
        ...
        if [ -z "$SRC" ]; then
            ...
            if /usr/bin/ditto "$SRC" "$STAGE"; then
                ...
                VERIFY_REQ='identifier "com.kryon.utils" and anchor apple generic and certificate leaf[subject.OU] = "3D485NHW29"'
                note fail-verify
    ...
    static func elevatedInstallCommand(appPath: String,
        ...
        let args = [appPath, dmgPath, "\(pid)", resultPath, "\(uid)"]
            ...
            .joined(separator: " ")
        return "/usr/bin/nohup /bin/sh -c \(script) kryon-installer \(args) >/dev/null 2>&1 &"
    }
