...
enum CommandBarCatalog {
    ...
    /// showing its own shortcut. This is how the bar reaches past Kryon
    ...
    /// runs, so one row serves both "gh" and "gh kryon".
    ...
    private static func keepOnShelf(_ text: String) {
        let board = NSPasteboard(name: NSPasteboard.Name("com.kryon.commandbar.selection"))
        board.clearContents()
