...
enum AdminShell {
    // Kryon is a menu-bar agent (LSUIElement), so it is rarely the active
...
enum Sudoers {
    static let rulePath = "/etc/sudoers.d/kryon-clamshell"
    ...
    private static let legacyRulePaths = [
        "/etc/sudoers.d/kryon-utils-clamshell",
    ...
    private static let sleepStateQueue = DispatchQueue(label: "com.kryon.utils.pmset-state")
