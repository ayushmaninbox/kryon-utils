...
final class StatusItemController {
    ...
    private static let mainAutosaveName = "KryonMenuBarItem"
    private static let metricAutosavePrefix = "KryonMetric"
    ...
    private func installStatusItem() {
        ...
        if let button = statusItem.button {
            button.image = BlackHoleGlyph.image(active: false, appearance: button.effectiveAppearance)
    ...
    private func updateIconAppearance() {
        ...
        let stateKey = [String(hidden), String(mainItemHidden), String(updateAvailable),
                        ...
                        String(micBadgeActive), String(isDarkMenuBar)].joined(separator: "|")
        ...
        if updateAvailable {
            stateImage = BlackHoleGlyph.attentionImage(appearance: button.effectiveAppearance)
        } else {
            stateImage = BlackHoleGlyph.image(active: keepAwakeActive, appearance: button.effectiveAppearance)
        }
        if micBadgeActive {
...
enum BlackHoleGlyph {
    ...
    static let pointSize = NSSize(width: 34, height: 24)
    ...
    /// alone would render blurry on Retina menu bars. Non-template: these keep
    /// the mark's real two-tone art (eyes, wrench highlight) instead of being
    /// collapsed to a flat silhouette.
    private static func loadTwoTone(_ prefix: String) -> NSImage? {
        let image = NSImage(size: pointSize)
        for resource in [prefix, "\(prefix)@2x"] {
    ...
    private static let baseBlack: NSImage? = loadTwoTone("MenuBarIconBlack")
    private static let baseWhite: NSImage? = loadTwoTone("MenuBarIconWhite")

    /// Picks the ink color that matches the menu bar's actual light/dark
    /// rendering. That can differ from the app's own appearance override (see
    /// AppAppearanceController), so callers pass the status item button's own
    /// `effectiveAppearance` rather than the app-wide one where possible.
    private static func base(for appearance: NSAppearance) -> NSImage? {
        let isDark = appearance.bestMatch(from: [.aqua, .darkAqua]) == .darkAqua
        return isDark ? (baseWhite ?? baseBlack) : (baseBlack ?? baseWhite)
    }

    static func image(active: Bool, appearance: NSAppearance = NSApp.effectiveAppearance) -> NSImage? {
        let tint = KeepAwakeIconTint.current
        guard active else { return base(for: appearance) ?? fallback(active: false) }
        return activeImage(style: .current, tint: tint, appearance: appearance)
    }

    static func activeImage(style: KeepAwakeActiveIcon,
                            tint: KeepAwakeIconTint = .orange,
                            appearance: NSAppearance = NSApp.effectiveAppearance) -> NSImage? {
        if let symbolName = style.systemSymbolName {
            guard let source = fixedSizeSymbol(named: symbolName, drop: style.menuBarDrop) else {
                return fallback(active: tint != .none)
            }
        ...
        // The mascot itself: keep its real two-tone art unless a tint color
        // asks for a flat accent instead.
        guard let source = base(for: appearance) else { return fallback(active: tint != .none) }
        guard let color = color(for: tint) else { return source }
        return tintedImage(source, color: color) ?? fallback(active: true)
    }
    ...
    static func attentionImage(appearance: NSAppearance = NSApp.effectiveAppearance) -> NSImage? {
        guard let source = base(for: appearance) else { return fallback(active: true) }
        return tintedImage(source, color: .systemBlue) ?? fallback(active: true)
    }
