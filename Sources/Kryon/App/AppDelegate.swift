...
final class AppDelegate: NSObject, NSApplicationDelegate, NSPopoverDelegate, NSWindowDelegate {
    ...
    private static let menuBarLog = Logger(subsystem: Bundle.main.bundleIdentifier ?? "kryon",
                                           category: "menubar")
    ...
    func relaunchApp() {
        ...
        task.arguments = ["-c", "sleep 0.3; /usr/bin/open \"$1\"", "kryon-relaunch", path]
        try? task.run()
        NSApp.terminate(nil)
    }
    ...
    /// On launch after an update, show the feature showcase once per version.
