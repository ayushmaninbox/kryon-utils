...
APP="/Applications/Kryon (Developer).app"
PROCESS="KryonDeveloper"
...
sleep 1.5
QP=$(ax 'get position of window "Kryon"')
if [[ -n "${QP:-}" ]]; then
...
sleep 1.5
SW=$(ax 'get position of window "Kryon Settings"')
if [[ -n "${SW:-}" ]]; then
    pass "settings window at $SW"
    screencapture -x "$OUT/settings.png"
    ax 'click button 1 of window "Kryon Settings"' >/dev/null
else
